...
(function(){
  ...
  const DEFAULT_DAY_DISPLAY={'Présence':{abbr:'ST',color:'#dbeafe'},'Congé annuel':{abbr:'CA',color:'#bbf7d0'},'RTT':{abbr:'RTT',color:'#93c5fd'},'Repos':{abbr:'RH',color:'#eef1f4'},'Jour férié':{abbr:'RFE',color:'#fef08a'},'Maladie':{abbr:'MAL',color:'#fecaca'},'Formation':{abbr:'F',color:'#ede9fe'},'Décès / deuil':{abbr:'DEC',color:'#ede9fe'},'Mariage / PACS':{abbr:'MAR',color:'#ede9fe'},'Enfant malade':{abbr:'EM',color:'#fecaca'},'Naissance / adoption':{abbr:'NA',color:'#ede9fe'},'Autorisation d’absence':{abbr:'AA',color:'#ede9fe'},'Absence temps partiel':{abbr:'RTP',color:'#ede9fe'}};
  ...
  function renderChronoDayRules(){ensureData();const box=document.getElementById('chronoDayRuleSettings');if(!box)return;const rows=knownDayTypes().map(type=>{const rr=db.settings.chronoDayRules?.[type]||{mode:'planned',hours:null},dd=db.settings.chronoDayDisplay?.[type]||{abbr:type.slice(0,3).toUpperCase(),color:'#ede9fe'},locked=type==='Maladie';return `<div class="chrono-rule-row" data-chrono-rule-type="${esc(type)}"><strong>${esc(type)}</strong><label>Abréviation<input data-chrono-abbr maxlength="6" value="${esc(type==='Absence temps partiel'?'RTP':dd.abbr||'')}"></label><label>Couleur<input type="color" data-chrono-color value="${esc(dd.color||'#ede9fe')}"></label><label>Comptabilisation<select data-chrono-rule-mode ${locked?'disabled':''}><option value="planned" ${rr.mode==='planned'?'selected':''}>Horaires prévus</option><option value="fixed" ${rr.mode==='fixed'?'selected':''}>Nombre d’heures fixe</option><option value="zero" ${rr.mode==='zero'?'selected':''}>0 h</option></select></label><label class="chrono-fixed-hours ${rr.mode==='fixed'?'':'hidden'}">Heures <input type="number" min="0" max="24" step="0.25" data-chrono-rule-hours value="${esc(rr.hours??'')}"></label><small>${locked?'Règle imposée : Maladie = 7 h':'Définit la pastille et le calcul de la journée.'}</small></div>`}).join('');box.innerHTML=rows||'<p>Aucun type.</p>';box.querySelectorAll('[data-chrono-rule-mode]').forEach(sel=>sel.addEventListener('change',()=>{const row=sel.closest('[data-chrono-rule-type]'),type=row.dataset.chronoRuleType;db.settings.chronoDayRules[type]=db.settings.chronoDayRules[type]||{};db.settings.chronoDayRules[type].mode=sel.value;if(sel.value==='fixed'&&db.settings.chronoDayRules[type].hours==null)db.settings.chronoDayRules[type].hours=7;row.querySelector('.chrono-fixed-hours')?.classList.toggle('hidden',sel.value!=='fixed');save()}));box.querySelectorAll('[data-chrono-rule-hours]').forEach(inp=>inp.addEventListener('change',()=>{const row=inp.closest('[data-chrono-rule-type]'),type=row.dataset.chronoRuleType;db.settings.chronoDayRules[type]={mode:'fixed',hours:Math.max(0,Number(inp.value||0))};if(type==='Maladie')db.settings.chronoDayRules[type]={mode:'fixed',hours:7};save()}));box.querySelectorAll('[data-chrono-abbr]').forEach(inp=>inp.addEventListener('change',()=>{const row=inp.closest('[data-chrono-rule-type]'),type=row.dataset.chronoRuleType;db.settings.chronoDayDisplay[type]=db.settings.chronoDayDisplay[type]||{};db.settings.chronoDayDisplay[type].abbr=type==='Absence temps partiel'?'RTP':String(inp.value||'').trim().toUpperCase().slice(0,6);if(type==='Absence temps partiel')inp.value='RTP';save()}));box.querySelectorAll('[data-chrono-color]').forEach(inp=>inp.addEventListener('change',()=>{const row=inp.closest('[data-chrono-rule-type]'),type=row.dataset.chronoRuleType;db.settings.chronoDayDisplay[type]=db.settings.chronoDayDisplay[type]||{};db.settings.chronoDayDisplay[type].color=inp.value;save()}))}
