...
(function(){
  ...
  async function applyChrono(){
    ...
    db.importArchives=db.importArchives||[];const lastChrono=db.pdfImports[db.pdfImports.length-1];db.importArchives.push({id:uid(),sourceId:lastChrono.id,createdAt:lastChrono.createdAt,type:'Chronotime',fileName:lastChrono.fileName,attachmentId:lastChrono.attachmentId,subject:lastChrono.subject,academicYear:lastChrono.academicYear,summary:lastChrono.summary,module:'pdfimports',analysisSnapshot:{type:'Chronotime',fileName:lastChrono.fileName,subject:lastChrono.subject,academicYear:p.academicYear,period:`${p.start||''} → ${p.end||''}`,daysRead:p.records.length,daysExpected:p.expectedDays||p.records.length,durationDays:p.durationDays||0,presence:minutesToDuration(p.totals.presence).replace(/^\+/,''),reference:minutesToDuration(p.totals.reference).replace(/^\+/,''),delta:minutesToDuration(p.totals.delta),ca:p.codeCounts?.CA||0,rtt:p.codeCounts?.RTT||0,rh:p.codeCounts?.RH||0,rfe:p.codeCounts?.RFE||0,confidence:p.confidence||0,informationCoverage:p.infoCoverage||0,calendarCoverage:p.coverage||0,missingInfo:[...(p.missingInfo||[])],codeCounts:{...(p.codeCounts||{})},monthly:p.monthly||{},totalSources:p.totalSources||{},records:p.records.map(r=>({date:r.date,value:r.value,duration:r.duration}))}});
