...
(function(){
  ...
  const DEFAULT_DAY_DISPLAY={
    'Présence':{abbr:'ST',color:'#dbeafe'},'Congé annuel':{abbr:'CA',color:'#bbf7d0'},'RTT':{abbr:'RTT',color:'#93c5fd'},
    'Repos':{abbr:'—',color:'#eef1f4'},'Jour férié':{abbr:'JF',color:'#fef08a'},'Maladie':{abbr:'MAL',color:'#fecaca'},
    'Formation':{abbr:'F',color:'#ede9fe'},'Décès / deuil':{abbr:'DEC',color:'#ede9fe'},'Mariage / PACS':{abbr:'MAR',color:'#ede9fe'},
    'Enfant malade':{abbr:'EM',color:'#fecaca'},'Naissance / adoption':{abbr:'NA',color:'#ede9fe'},'Autorisation d’absence':{abbr:'AA',color:'#ede9fe'},
    'Absence temps partiel':{abbr:'RTP',color:'#ede9fe'}
  };
  ...
  function ensureData(){
    ...
    db.settings.chronoDayDisplay['Absence temps partiel']=Object.assign({},db.settings.chronoDayDisplay['Absence temps partiel']||{}, {abbr:'RTP'});
    // Maladie est toujours comptabilisée à 7 h, règle métier explicite.
    db.settings.chronoDayRules['Maladie']={mode:'fixed',hours:7};
  ...
  function renderCodeMap(){ensureData();const box=document.getElementById('chronoCodeMap');if(box)box.innerHTML=Object.entries(db.settings.chronoCodeMap).sort().map(([k,v])=>`<span class="code-map-item"><strong>${esc(k)}</strong><span>${esc(v)}</span><small>${esc(ruleLabel(db.settings.chronoDayRules?.[v]))}</small></span>`).join('')||'<p>Aucun code enregistré.</p>';renderChronoDayRules()}
  function renderChronoDayRules(){
    ensureData();const box=document.getElementById('chronoDayRuleSettings');if(!box)return;
    const rows=knownDayTypes().map(type=>{const rule=db.settings.chronoDayRules?.[type]||{mode:'planned',hours:null};const disp=db.settings.chronoDayDisplay?.[type]||{abbr:(type==='Absence temps partiel'?'RTP':type.slice(0,3).toUpperCase()),color:'#ede9fe'};const locked=type==='Maladie';return `<div class="chrono-rule-row chrono-rule-row-v94" data-chrono-rule-type="${esc(type)}"><strong>${esc(type)}</strong><label>Abréviation<input type="text" maxlength="6" data-chrono-abbr value="${esc(type==='Absence temps partiel'?'RTP':disp.abbr||'')}"></label><label>Couleur<input type="color" data-chrono-color value="${esc(disp.color||'#ede9fe')}"></label><label>Comptabilisation<select data-chrono-rule-mode ${locked?'disabled':''}><option value="planned" ${rule.mode==='planned'?'selected':''}>Horaires prévus</option><option value="fixed" ${rule.mode==='fixed'?'selected':''}>Nombre d’heures fixe</option><option value="zero" ${rule.mode==='zero'?'selected':''}>0 h</option></select></label><label class="chrono-fixed-hours ${rule.mode==='fixed'?'':'hidden'}">Heures <input type="number" min="0" max="24" step="0.25" data-chrono-rule-hours value="${esc(rule.hours??'')}"></label><small>${locked?'Règle imposée : Maladie = 7 h':'Ces réglages pilotent les pastilles du calendrier.'}</small></div>`}).join('');box.innerHTML=rows||'<p>Aucun type de journée.</p>';
    box.querySelectorAll('[data-chrono-rule-mode]').forEach(sel=>sel.addEventListener('change',()=>{const row=sel.closest('[data-chrono-rule-type]'),type=row.dataset.chronoRuleType;db.settings.chronoDayRules[type]=db.settings.chronoDayRules[type]||{};db.settings.chronoDayRules[type].mode=sel.value;if(sel.value==='fixed'&&db.settings.chronoDayRules[type].hours==null)db.settings.chronoDayRules[type].hours=7;row.querySelector('.chrono-fixed-hours')?.classList.toggle('hidden',sel.value!=='fixed');save();renderCodeMap()}));
    box.querySelectorAll('[data-chrono-rule-hours]').forEach(inp=>inp.addEventListener('change',()=>{const row=inp.closest('[data-chrono-rule-type]'),type=row.dataset.chronoRuleType;db.settings.chronoDayRules[type]=db.settings.chronoDayRules[type]||{mode:'fixed'};db.settings.chronoDayRules[type].hours=Math.max(0,Number(inp.value||0));if(type==='Maladie')db.settings.chronoDayRules[type]={mode:'fixed',hours:7};save();renderCodeMap()}));
    box.querySelectorAll('[data-chrono-abbr]').forEach(inp=>inp.addEventListener('change',()=>{const row=inp.closest('[data-chrono-rule-type]'),type=row.dataset.chronoRuleType;db.settings.chronoDayDisplay[type]=db.settings.chronoDayDisplay[type]||{};db.settings.chronoDayDisplay[type].abbr=type==='Absence temps partiel'?'RTP':String(inp.value||'').trim().toUpperCase().slice(0,6);if(type==='Absence temps partiel')inp.value='RTP';save()}));
    box.querySelectorAll('[data-chrono-color]').forEach(inp=>inp.addEventListener('change',()=>{const row=inp.closest('[data-chrono-rule-type]'),type=row.dataset.chronoRuleType;db.settings.chronoDayDisplay[type]=db.settings.chronoDayDisplay[type]||{};db.settings.chronoDayDisplay[type].color=inp.value;save()}));
  }
  ...
  function chronoPreview(p){
    ...
    const unknown=p.unknownCodes.length?`<div class="import-warning"><strong>Nouveaux types Chronotime détectés</strong><p class="muted">Pour chaque nouveau code, choisissez son appellation et la façon dont la journée doit être comptabilisée. Ce choix sera mémorisé.</p>${p.unknownCodes.map(c=>`<div class="chrono-new-code" data-new-chrono-code="${esc(c)}"><strong>Code ${esc(c)}</strong><label>Appellation<input data-chrono-code="${esc(c)}" list="chronoDayTypeSuggestions" placeholder="Ex. Enfant malade"></label><label>Abréviation de pastille<input data-chrono-new-abbr maxlength="6" placeholder="Ex. RTP"></label><label>Couleur de pastille<input type="color" data-chrono-new-color value="#ede9fe"></label><label>Comptabilisation<select data-chrono-new-rule><option value="">Choisir…</option><option value="planned">Horaires prévus</option><option value="fixed">Nombre d’heures fixe</option><option value="zero">0 h</option></select></label><label class="chrono-new-fixed hidden">Heures fixes<input type="number" min="0" max="24" step="0.25" data-chrono-new-hours></label></div>`).join('')}<datalist id="chronoDayTypeSuggestions">${knownDayTypes().map(v=>`<option value="${esc(v)}"></option>`).join('')}</datalist></div>`:'';
  ...
  async function applyChrono(){
    ...
    save();renderCodeMap();renderHistory();renderDashboard();status('chronoImportStatus',`Injection terminée : ${p.records.length} jours · ${importChanges.length} modification${importChanges.length>1?'s':''} de type de journée · ${absences} codes/absences · ${durations} présences.`, 'ok');document.getElementById('chronoPreview').innerHTML='';chronoPending=null;if(typeof clearAcademicYearMismatch==='function')clearAcademicYearMismatch();
  }
