...
(function(){
  ...
  async function applyControl(){
    ...
    for(let i=0;i<p.candidates.length;i++){
      const c=p.candidates[i],pri=document.querySelector(`[data-control-priority="${i}"]`)?.value||c.priority||'Normale',st=document.querySelector(`[data-control-status="${i}"]`)?.value||c.status||'À traiter';
      ...
      if(createMaint&&checked&&st==='À traiter'){db.maintenance.push({id:uid(),no:nextNo('maintenance','MAI'),date:p.reportDate||todayISO(),title:`Observation ${nc.observationNo} — ${p.organization}`,family:controlFamily,priority:pri,status:'À faire',building:'',floor:'',room:c.location||'',requester:p.organization,assigned:'',dueDate:'',cost:0,description:c.text,action:c.action?`Préconisation : ${c.action}`:'À corriger puis faire lever la non-conformité',attachments:[attachment],sourceNonconformityId:nc.id});created++}
    }
    db.pdfImports.push({id:reportId,kind:'control',importType:p.centralKind==='periodic'?'periodic':'report',createdAt:new Date().toISOString(),fileName:p.file.name,fileSize:p.file.size,fileHash,attachmentId:attachment.id,subject:p.organization,controlFamily,reportDate:p.reportDate,periodicControlId:periodicTargetId,academicYear:academicYearFor(p.reportDate||todayISO()),summary:`${p.candidates.filter(c=>!['FAIT','Levée'].includes(c.status)).length} à traiter · ${p.candidates.filter(c=>['FAIT','Levée'].includes(c.status)).length} FAIT/levées · ${created} intervention(s)`});
    db.importArchives=db.importArchives||[];const lastControl=db.pdfImports[db.pdfImports.length-1];const controlArchive={id:uid(),sourceId:lastControl.id,createdAt:lastControl.createdAt,type:p.centralKind==='periodic'?'Contrôle périodique':'Rapport de contrôle',fileName:lastControl.fileName,fileHash:lastControl.fileHash||'',attachmentId:lastControl.attachmentId,subject:lastControl.subject,academicYear:lastControl.academicYear,summary:lastControl.summary,module:'periodic',recordId:periodicTargetId||'',analysisSnapshot:{type:p.centralKind==='periodic'?'Contrôle périodique':'Rapport de contrôle',fileName:lastControl.fileName,subject:p.organization,organization:p.organization,controlFamily,reportDate:p.reportDate,academicYear:lastControl.academicYear,confidence:p.confidence||0,nonconformities:p.candidates.filter(c=>!['FAIT','Levée'].includes(c.status)).length,maintenanceCreated:created,periodicControlId:periodicTargetId,candidates:p.candidates.map((c,i)=>({observationNo:c.observationNo||i+1,location:c.location||'',text:c.text,action:c.action||'',status:document.querySelector(`[data-control-status="${i}"]`)?.value||c.status||'À traiter',priority:document.querySelector(`[data-control-priority="${i}"]`)?.value||c.priority,selected:document.querySelector(`[data-control-select="${i}"]`)?.checked!==false})),summary:lastControl.summary}};db.importArchives.push(controlArchive);if(attachment)window.PSTImportOriginals?.remember?.(controlArchive,attachment);
    save();renderHistory();renderControlLibrary();status('controlImportStatus',`Rapport archivé : ${p.candidates.filter(c=>!['FAIT','Levée'].includes(c.status)).length} à traiter, ${p.candidates.filter(c=>['FAIT','Levée'].includes(c.status)).length} FAIT/levées, ${created} intervention(s).`,'ok');document.getElementById('controlPreview').innerHTML='';controlPending=null;renderAll();
  }
