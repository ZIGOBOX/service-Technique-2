...
(function(){
  ...
  async function applyControl(){
    ...
    if(periodicTargetId!=='__create__'&&periodicTargetId && !(db.periodic||[]).some(x=>String(x.id)===String(periodicTargetId))){
      status('controlImportStatus','Contrôle périodique cible introuvable. Aucun PDF n’a été envoyé.','error');
      return;
    }
    const reportId=uid();
    let attachment=null;
    ...
    let persisted=window.PSTMainState?.persistNow?await window.PSTMainState.persistNow():{ok:save(),offline:!navigator.onLine};
if(!persisted?.ok && navigator.onLine){
 await new Promise(res=>setTimeout(res,700));
 persisted=window.PSTMainState?.persistNow?await window.PSTMainState.persistNow():persisted;
}
renderHistory();renderControlLibrary();
if(!persisted?.ok&&!persisted?.offline){
 status('controlImportStatus','⚠️ Le PDF est bien dans Supabase Storage, mais la fiche n’est pas encore confirmée dans Supabase. Ne rechargez pas la page ; utilisez Requête Supabase puis réessayez.','error');
 toast?.('PDF chargé, fiche en attente de confirmation Supabase');
 return;
}
status('controlImportStatus',`✅ Rapport enregistré${persisted?.offline?' hors ligne — synchronisation automatique':' et synchronisé'} : ${p.candidates.filter(c=>!['FAIT','Levée'].includes(c.status)).length} à traiter · ${actionCreated} plan(s) d’action · ${created} intervention(s).`,'ok');document.getElementById('controlPreview').innerHTML='';controlPending=null;const pf=document.getElementById('periodicFamily'),ps=document.getElementById('periodicStatus'),pb=document.getElementById('periodicBuilding');if(pf)pf.value='';if(ps)ps.value='';if(pb)pb.value='';const im=document.getElementById('issueMonth'),ia=document.getElementById('issueAgent'),ic=document.getElementById('issueCategory'),is=document.getElementById('issueStatus');if(im)im.value=todayISO().slice(0,7);if(ia)ia.value='';if(ic)ic.value='';if(is)is.value='';renderAll();if(typeof setView==='function')setView('periodic');setTimeout(()=>{document.getElementById('periodicCards')?.scrollIntoView({behavior:'smooth',block:'start'});toast?.(`Rapport enregistré : ${actionCreated} plan(s) d’action créé(s)`);},80);
  }
  ...
  async function reattachControlReport(reportId,file){if(!file)return;const report=(db.pdfImports||[]).find(x=>x.id===reportId);if(!report)return;status('controlImportStatus','Rattachement du document original…');try{const attachment=await putFile(file,{module:'control-report',recordId:reportId});db.attachments.push(attachment);report.attachmentId=attachment.id;report.fileName=file.name||report.fileName;report.fileSize=file.size;report.fileHash=await fileFingerprint(file);const archive=(db.importArchives||[]).find(x=>String(x.sourceId)===String(reportId));
if(archive){archive.attachmentId=attachment.id;window.PSTImportOriginals?.remember?.(archive,attachment)}
const periodic=(db.periodic||[]).find(x=>String(x.id)===String(report.periodicControlId||archive?.recordId||''));
if(periodic){
 periodic.attachments=periodic.attachments||[];
 const idx=periodic.attachments.findIndex(a=>!a?.storagePath&&normalizeText(a?.name||'')===normalizeText(report.fileName||file.name||''));
 if(idx>=0)periodic.attachments[idx]=attachment;
 else if(!periodic.attachments.some(a=>String(a.id)===String(attachment.id)))periodic.attachments.push(attachment);
}
const persisted=window.PSTMainState?.persistNow?await window.PSTMainState.persistNow():{ok:save(),offline:!navigator.onLine};
renderControlLibrary();renderPeriodic?.();
if(!persisted?.ok&&!persisted?.offline)throw new Error('Le fichier a été chargé mais la fiche n’a pas été confirmée dans Supabase');
status('controlImportStatus',persisted?.offline?'Original rattaché — synchronisation en attente.':'Original rattaché et confirmé. Il peut maintenant être rouvert partout.','ok')}catch(e){console.error(e);status('controlImportStatus',`Impossible de rattacher l’original : ${e?.message||String(e)}`,'error')}}
  ...
  function refreshImportedReports(){try{ensureData();renderHistory();renderDashboard();renderControlLibrary()}catch(e){console.error('Rafraîchissement rapports périodiques',e)}}
  function init(){ensureData();bind();renderHistory();renderCodeMap();renderDashboard();renderControlLibrary();window.addEventListener('pst:data-loaded',refreshImportedReports);window.addEventListener('pst:data-saved',refreshImportedReports)}
