...
(function(){
  ...
  function parseControl(extracted,file){
    ...
    return {kind:'control',file,extracted,organization:org,reportDate,candidates:candidates.slice(0,100)};
  }
  ...
  function controlPreview(p){
    ...
    box.innerHTML=`<div class="import-summary"><strong>${esc(p.file.name)}</strong><span>Organisme : ${esc(p.organization)}</span><span>Date détectée : ${fmtDate(p.reportDate)}</span><span>Non-conformités proposées : ${p.candidates.length}</span></div><label class="inline-check"><input type="checkbox" id="controlCreateMaintenance" checked> Créer une intervention de maintenance pour chaque non-conformité validée</label><div class="control-candidate-list">${list}</div><div class="modal-actions inline-actions"><button class="ghost" id="cancelControlImport">Annuler</button><button class="primary" id="applyControlImport">Valider le rapport</button></div>`;
  ...
  async function applyControl(){
    const p=controlPending;if(!p)return;const selected=p.candidates.filter((c,i)=>document.querySelector(`[data-control-select="${i}"]`)?.checked);const createMaint=document.getElementById('controlCreateMaintenance')?.checked;
    ...
    let created=0;
    selected.forEach((c,i)=>{const pri=document.querySelector(`[data-control-priority="${p.candidates.indexOf(c)}"]`)?.value||c.priority;const nc={id:uid(),no:`NC-${String((db.reportNonconformities?.length||0)+1).padStart(4,'0')}`,organization:p.organization,reportDate:p.reportDate,text:c.text,priority:pri,status:'À traiter',sourceFile:p.file.name,attachmentId:attachment?.id||'',createdAt:new Date().toISOString()};db.reportNonconformities.push(nc);if(createMaint){db.maintenance.push({id:uid(),no:nextNo('maintenance','MAI'),date:p.reportDate||todayISO(),title:`Non-conformité ${p.organization}`,family:'Autre',priority:pri,status:'À faire',building:'',floor:'',room:'',requester:p.organization,assigned:'',dueDate:'',cost:0,description:c.text,action:'À corriger puis faire lever la non-conformité',attachments:attachment?[attachment]:[],sourceNonconformityId:nc.id});created++;}});
    db.pdfImports.push({id:uid(),kind:'control',createdAt:new Date().toISOString(),fileName:p.file.name,attachmentId:attachment?.id||'',subject:p.organization,academicYear:academicYearFor(p.reportDate||todayISO()),summary:`${selected.length} non-conformité(s) validée(s) · ${created} intervention(s)`});save();renderHistory();status('controlImportStatus',`Rapport validé : ${selected.length} non-conformité(s), ${created} intervention(s) créée(s).`,'ok');document.getElementById('controlPreview').innerHTML='';controlPending=null;renderAll();
  }
  ...
  function bind(){const c=document.getElementById('chronoPdfFile'),r=document.getElementById('controlPdfFile');if(c)c.addEventListener('change',e=>onChronoFile(e.target.files?.[0]));if(r)r.addEventListener('change',e=>onControlFile(e.target.files?.[0]));document.addEventListener('click',e=>{const go=e.target.closest('[data-go="pdfimports"]');if(go){e.preventDefault();setView('pdfimports');renderHistory();renderCodeMap();}})}
  function init(){ensureData();bind();renderHistory();renderCodeMap();renderDashboard()}
  window.PDFImportModule={init,renderHistory,renderCodeMap,renderDashboard};
