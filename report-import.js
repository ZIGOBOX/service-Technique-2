  ...
  async function applyControl(){
    ...
    for(let i=0;i<p.candidates.length;i++){
      ...
      if(checked&&st==='À traiter'){
        db.issues=db.issues||[];
        db.issues.push({id:uid(),no:nextNo('issue','ACT'),date:p.reportDate||todayISO(),agentId:'',category:'Sécurité',title:`Non-conformité — Observation ${nc.observationNo} — ${p.organization}`,description:[c.location?`Localisation : ${c.location}`:'',c.text].filter(Boolean).join('\n'),priority:pri,status:'À faire',owner:'',dueDate:'',cost:0,action:c.action?`Préconisation : ${c.action}`:'À corriger puis faire lever la non-conformité',attachments:[attachment],sourceNonconformityId:nc.id,sourceReportId:reportId,periodicControlId:periodicTargetId});
        actionCreated++;
      }
      if(createMaint&&checked&&st==='À traiter'){db.maintenance.push({id:uid(),no:nextNo('maintenance','MAI'),date:p.reportDate||todayISO(),title:`Observation ${nc.observationNo} — ${p.organization}`,family:controlFamily,priority:pri,status:'À faire',building:'',floor:'',room:c.location||'',requester:p.organization,assigned:'',dueDate:'',cost:0,description:c.text,action:c.action?`Préconisation : ${c.action}`:'À corriger puis faire lever la non-conformité',attachments:[attachment],sourceNonconformityId:nc.id,periodicControlId:periodicTargetId});created++}
    }
