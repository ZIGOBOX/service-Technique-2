...
(function(){
  ...
  function renderChronoDayRules(){ensureData();const box=document.getElementById('chronoDayRuleSettings');if(!box)return;const rows=knownDayTypes().map(type=>{const r=db.settings.chronoDayRules?.[type]||{mode:'planned',hours:null};const locked=type==='Maladie';return `<div class="chrono-rule-row" data-chrono-rule-type="${esc(type)}"><strong>${esc(type)}</strong><select data-chrono-rule-mode ${locked?'disabled':''}><option value="planned" ${r.mode==='planned'?'selected':''}>Horaires prévus</option><option value="fixed" ${r.mode==='fixed'?'selected':''}>Nombre d’heures fixe</option><option value="zero" ${r.mode==='zero'?'selected':''}>0 h</option></select><label class="chrono-fixed-hours ${r.mode==='fixed'?'':'hidden'}">Heures <input type="number" min="0" max="24" step="0.25" data-chrono-rule-hours value="${esc(r.hours??'')}"></label><small>${locked?'Règle imposée : Maladie = 7 h':'Définit comment cette journée est comptabilisée.'}</small></div>`}).join('');box.innerHTML=rows||'<p>Aucun type de journée.</p>';box.querySelectorAll('[data-chrono-rule-mode]').forEach(sel=>sel.addEventListener('change',()=>{const row=sel.closest('[data-chrono-rule-type]'),type=row.dataset.chronoRuleType;db.settings.chronoDayRules[type]=db.settings.chronoDayRules[type]||{};db.settings.chronoDayRules[type].mode=sel.value;if(sel.value==='fixed'&&db.settings.chronoDayRules[type].hours==null)db.settings.chronoDayRules[type].hours=7;row.querySelector('.chrono-fixed-hours')?.classList.toggle('hidden',sel.value!=='fixed');save();renderCodeMap()}));box.querySelectorAll('[data-chrono-rule-hours]').forEach(inp=>inp.addEventListener('change',()=>{const row=inp.closest('[data-chrono-rule-type]'),type=row.dataset.chronoRuleType;db.settings.chronoDayRules[type]=db.settings.chronoDayRules[type]||{mode:'fixed'};db.settings.chronoDayRules[type].hours=Math.max(0,Number(inp.value||0));if(type==='Maladie')db.settings.chronoDayRules[type]={mode:'fixed',hours:7};save();renderCodeMap()}))}
