...
autoDailyEnabled:true,autoWeeklyEnabled:false,autoReportHour:'07:00',autoReportTimezone:'Europe/Paris',autoReportWeekdays:'1,2,3,4,5',autoReportOnlyIfEvents:false,autoReportIncludeAgents:true,autoReportIncludeMaintenance:true,autoReportIncludeCleaning:true,autoReportIncludePeriodic:true,autoReportIncludeMeetings:true,autoReportSignature:'Rapport généré automatiquement par Pilotage Service Technique.',autoReportTo:'adelin.vignal.running@outlook.fr',autoReportCc:'',autoReportBcc:'',autoReportServerProvider:'',autoReportServerSender:'',autoReportServerCron:false,lastAutoReportCheckAt:'',lastAutoReportTestAt:'',lastAutoReportSentAt:'',lastAutoReportType:'',lastAutoReportStatus:'',lastAutoReportError:'',lastDailyEmailDate:'',lastWeeklyEmailKey:'',lastWeeklyArchiveKey:'',lastAnnualResetYear:0,appName:'Pilotage Service Technique',schoolName:'Lycée Jean Puy',schoolZone:'A',academicYear:academicYearFor(todayISO()),defaultLayout:'auto',printOrientation:'landscape',defaultInspector:'',emailsTo:'',emailsCc:'',emailsBcc:'',emailSubjectPrefix:'Pilotage Service Technique',outlookEmail:'',counters:{}},lists:clone(defaultLists),buildings,spaces:defaultSpaces(buildings),agents,weeklyPlans:clone(IMPORTED_WEEKLY_PLANS),rotations:[],rotationExceptions:[],agentDays:[],personalEvents:[],roomPreps:[],agentActivities:[],issues:[],periodic:makeContractControls14723(),cleaning:[],maintenance,requests:[],works:[],meetings:[],notes:[],vacations:[],documents:[],oneDriveLinks:[],contracts:[],contacts:[],attachments:[],archives:[],importArchives:[],cleaningRoomsConfig:null,cleaningRoomChecks:[],cleaningDeletedIds:[],notificationDismissals:{},importOriginalBindings:{}}}
...
function migrate(raw){
 ...
 // V147.170 — destination dédiée des rapports automatiques. Elle reste indépendante des e-mails manuels.
 if(!String(d.settings.autoReportTo||'').trim())d.settings.autoReportTo='adelin.vignal.running@outlook.fr';
...
function dashboardAgentWeekCellV150(agent,date){
 const info=dayInfo(agent.id,date),type=String(info.dayType||'Présence'),norm=normalizeText(type);
 const hasReal=!!(info.actualStart&&info.actualEnd),start=String(hasReal?info.actualStart:info.plannedStart||''),end=String(hasReal?info.actualEnd:info.plannedEnd||'');
 ...
 else if(start&&end){main=`${start}–${end}`;sub=hasReal?'Horaire réel':(info.shift&&normalizeText(info.shift)!=='standard'?String(info.shift):'');}
 const h=dayHours(info),displayHours=hasReal?Number(h.actual||0):Number(h.planned||0);
 return {html:`<button type="button" class="agent-week-cell-v150 ${cls} ${hasReal?'real-schedule-v167':''} ${date===todayISO()?'today':''}" data-agent-day="${agent.id}" data-date="${date}" title="${esc(agentName(agent))} — ${fmtDate(date)}${hasReal?' — horaire réel':''}"><span>${esc(main)}</span>${sub?`<small>${esc(sub)}</small>`:''}</button>`,hours:norm==='repos'||isAbsenceType(type)?0:displayHours};
}
...
function dashboardRealScheduleEventsForDateV167(d){
 // Tableau de bord : dès qu'un horaire réel complet existe, il devient un élément de l'agenda du jour.
 // On part du dayRecord prioritaire afin d'éviter les anciens doublons de saisie.
 const rows=[];
 for(const a of (db.agents||[]).filter(x=>normalizeText(x.status)==='actif')){
   const rec=dayRecord(a.id,d),info=dayInfo(a.id,d);
   const realStart=String(info.actualStart||'').trim(),realEnd=String(info.actualEnd||'').trim();
   if(!rec||!realStart||!realEnd)continue;
   rows.push({id:rec.id,date:d,start:realStart,time:realStart,source:'agent-real-schedule',
     title:`${agentName(a)||'Agent'} · Horaire réel : ${realStart}–${realEnd}`,
     location:'',note:info.note||'',agentId:a.id,calendarInfo:`Horaire réel enregistré`});
 }
 return rows;
}
function dashboardEventsForDateV159(d){
 ...
 // Les horaires réels sont reconstruits ici sans condition de différence avec le théorique : s'ils existent, ils apparaissent toujours dans Ma journée.
 const rows=[...eventsForDate(d).filter(e=>e.source!=='agent-real-schedule'),...dashboardRealScheduleEventsForDateV167(d)],seen=new Set();
...
function renderDashboardTeamTodayV149(){
 ...
 if(dashboardPeriodModeV159==='week'){
  el.innerHTML=agents.length?agents.map(a=>{
   ...
   for(const d of range.dates){const info=dayInfo(a.id,d),type=String(info.dayType||'Présence'),norm=normalizeText(type);if(norm==='repos')continue;if(isAbsenceType(type)){absence++;reasons.push(type)}else{presence++;const h=dayHours(info);total+=info.actualStart&&info.actualEnd?Number(h.actual||0):Number(h.planned||0)}}
 ...
 el.innerHTML=agents.length?agents.map(a=>{const info=dayInfo(a.id,d),type=String(info.dayType||'Présence'),norm=normalizeText(type),hasReal=!!(info.actualStart&&info.actualEnd),start=String(hasReal?info.actualStart:info.plannedStart||''),end=String(hasReal?info.actualEnd:info.plannedEnd||'');let status=hasReal?'Horaire réel':'Présent',cls=hasReal?'real-schedule-v167':'',schedule=start&&end?`${hasReal?'Réel · ':''}${start} – ${end}`:'Horaire non défini';if(norm==='repos'){status='Repos';cls='rest';schedule='Journée';}else if(isAbsenceType(type)){status=type;cls='absent';schedule='Journée';}else if(isToday&&start&&start>nowTime){status='À venir';cls=hasReal?'real-schedule-v167 future':'future';schedule=`${hasReal?'Réel · ':''}Prend à ${start}`;}return `<button class="dashboard-team-row-v149 ${hasReal?'real-schedule-row-v167':''}" data-agent-day="${a.id}" data-date="${d}"><span class="dashboard-team-avatar-v149">${esc((a.firstName||a.lastName||'?').charAt(0).toUpperCase())}</span><strong>${esc(agentName(a))}</strong><small>${esc(schedule)}</small><span class="dashboard-team-status-v149 ${cls}">${esc(status)}</span></button>`}).join(''):'<div class="empty-card">Aucun agent actif.</div>';
}
function renderDashboardTodayAgenda(){
 ...
 el.innerHTML=visible.length?visible.map(e=>{const tm=agendaTime(e),place=agendaPlace(e),day=dashboardPeriodModeV159==='week'?parseDate(e.__dashboardDate).toLocaleDateString('fr-FR',{weekday:'short',day:'2-digit'}):'',isReal=e.source==='agent-real-schedule';const meta=isReal?'🕒 Horaire réel enregistré':(day&&tm?`🕒 ${esc(tm)}${place?` · 📍 ${esc(place)}`:''}`:(place?`📍 ${esc(place)}`:'📍 Lieu non renseigné'));return `<button class="today-agenda-row agenda-action ${esc(e.source||'personal')} ${isReal?'real-schedule-row-v167':''}" data-agenda-source="${esc(e.source||'personal')}" data-agenda-id="${esc(e.id||'')}">${day?`<span class="today-agenda-day-v159">${esc(day)}</span>`:`<span class="today-agenda-time">${esc(tm||'—')}</span>`}<span class="today-agenda-main"><strong>${esc(e.title||'Événement')}</strong><small>${meta}</small></span><span class="today-agenda-arrow">›</span></button>`}).join(''):`<div class="empty">Aucun événement prévu ${dashboardPeriodModeV159==='week'?'cette semaine':'ce jour'}.</div>`;
}
function dashboardAgentWeekDatesV150(offset=0){
...
function automaticMailProviderLabel(provider=''){
 return provider==='microsoft'?'Microsoft 365 / Outlook':provider==='resend'?'Resend':provider==='none'?'Non configuré':'À vérifier';
}
function automaticMailWhenLabel(){
 const bits=[];if(db.settings.autoDailyEnabled)bits.push(`quotidien à ${db.settings.autoReportHour||'07:00'}`);if(db.settings.autoWeeklyEnabled)bits.push('hebdomadaire le lundi');return bits.length?bits.join(' + '):'aucun envoi automatique activé';
}
function renderAutomaticMailStatus(){
 const box=$('#automaticMailStatus');if(!box)return;
 const to=String(db.settings.autoReportTo||'adelin.vignal.running@outlook.fr').trim();
 const provider=automaticMailProviderLabel(db.settings.autoReportServerProvider||'');
 const providerOk=['microsoft','resend'].includes(db.settings.autoReportServerProvider||'');
 const cronKnown=Boolean(db.settings.lastAutoReportCheckAt);const cronOk=Boolean(db.settings.autoReportServerCron);
 const lastSent=db.settings.lastAutoReportSentAt?new Date(db.settings.lastAutoReportSentAt).toLocaleString('fr-FR'):'Aucun envoi confirmé';
 const lastTest=db.settings.lastAutoReportTestAt?new Date(db.settings.lastAutoReportTestAt).toLocaleString('fr-FR'):'Jamais testé';
 const err=String(db.settings.lastAutoReportError||'').trim();
 box.innerHTML=`<div class="mail-status-row"><span class="mail-status-icon ok">1</span><div><strong>Destination automatique</strong><small>${esc(to)}</small></div></div>
 <div class="mail-status-row"><span class="mail-status-icon ${providerOk?'ok':'warn'}">2</span><div><strong>Moteur d’envoi</strong><small>${esc(provider)}${db.settings.autoReportServerSender?` · expéditeur serveur : ${esc(db.settings.autoReportServerSender)}`:''}</small></div></div>
 <div class="mail-status-row"><span class="mail-status-icon ${cronKnown?(cronOk?'ok':'warn'):'warn'}">3</span><div><strong>Planification</strong><small>${esc(automaticMailWhenLabel())}${cronKnown?` · clé de planification serveur : ${cronOk?'configurée':'absente'}`:' · configuration serveur non vérifiée'}</small></div></div>
 <div class="mail-status-row"><span class="mail-status-icon ${err?'bad':db.settings.lastAutoReportSentAt?'ok':'neutral'}">4</span><div><strong>Dernier contrôle</strong><small>Test : ${esc(lastTest)} · Dernier envoi : ${esc(lastSent)}${err?` · Erreur : ${esc(err)}`:''}</small></div></div>`;
}
async function checkAutomaticMailConfiguration(showToast=true){
 const btn=$('#checkAutomaticMailConfiguration');if(btn)btn.disabled=true;
 try{
  saveSettings();
  if(!supabaseClient)throw new Error('Connexion Supabase indisponible');
  const {data,error}=await supabaseClient.functions.invoke('automatic-report',{body:{mode:'status'}});if(error)throw error;if(!data?.ok)throw new Error(data?.error||'Vérification impossible');if(typeof data.provider==='undefined')throw new Error('La fonction serveur automatic-report est une ancienne version : redéployez la fonction V147.170');
  db.settings.autoReportServerProvider=data.provider||'none';db.settings.autoReportServerSender=data.sender||'';db.settings.autoReportServerCron=Boolean(data.cronSecretConfigured);db.settings.lastAutoReportCheckAt=new Date().toISOString();db.settings.lastAutoReportError='';save();renderAutomaticMailStatus();
  if(showToast)toast(data.provider&&data.provider!=='none'?'Configuration e-mail serveur détectée':'Moteur d’envoi serveur non configuré');
  return data;
 }catch(e){db.settings.lastAutoReportCheckAt=new Date().toISOString();db.settings.lastAutoReportError=String(e?.message||e);save();renderAutomaticMailStatus();if(showToast)toast(`Vérification impossible : ${e?.message||e}`);throw e}finally{if(btn)btn.disabled=false}
}
async function sendAutomaticReportTest(){
 const btn=$('#sendAutomaticReportTest');if(btn)btn.disabled=true;
 try{
  saveSettings();
  const to=String(db.settings.autoReportTo||'').trim();if(!to)throw new Error('Renseignez l’adresse qui doit recevoir les rapports automatiques');
  if(!supabaseClient)throw new Error('Connexion Supabase indisponible');
  const {data,error}=await supabaseClient.functions.invoke('automatic-report',{body:{mode:'test'}});if(error)throw error;if(!data?.ok)throw new Error(data?.error||'Échec de l’envoi test');
  db.settings.lastAutoReportTestAt=new Date().toISOString();db.settings.lastAutoReportSentAt=data.sentAt||new Date().toISOString();db.settings.lastAutoReportType='test';db.settings.lastAutoReportStatus='ok';db.settings.lastAutoReportError='';if(data.provider)db.settings.autoReportServerProvider=data.provider;if(data.sender)db.settings.autoReportServerSender=data.sender;save();renderAutomaticMailStatus();toast(`Mail test envoyé à ${to}`);
 }catch(e){console.error(e);db.settings.lastAutoReportStatus='error';db.settings.lastAutoReportError=String(e?.message||e);save();renderAutomaticMailStatus();toast(`Envoi test impossible : ${e?.message||e}`)}finally{if(btn)btn.disabled=false}
}
...
document.addEventListener('click',async e=>{const ni=e.target.closest('[data-notification-index]');if(ni){const n=(window.__notifications||[])[Number(ni.dataset.notificationIndex)];closeNotificationCenter();if(n)notificationTarget(n);return}const ar=e.target.closest('[data-archive-detail]');if(ar){openArchiveDetail(ar.dataset.archiveDetail);return}const iana=e.target.closest('[data-open-import-analysis]');if(iana){openImportAnalysis(iana.dataset.openImportAnalysis);return}const irec=e.target.closest('[data-open-import-record]');if(irec){const id=irec.dataset.openImportRecord,m=irec.dataset.importModule;({notes:()=>openNote(id),issues:()=>openIssue(id),maintenance:()=>openMaintenance(id),requests:()=>openRequest(id),works:()=>openWork(id),meetings:()=>openMeeting(id),periodic:()=>openPeriodic(id),documents:()=>openDocument(id)}[m]||(()=>setView(m||'archives')))();return}const go=e.target.closest('[data-go]');if(go){if(go.closest('#dashboard'))dashboardShortcut(go.dataset.go);else setView(go.dataset.go);return}const quick=e.target.closest('[data-quick]');if(quick){dispatchQuick(quick.dataset.quick);return}const ae=e.target.closest('[data-agenda-source]');if(ae){
 ...
 else if(source==='agent-real-schedule'){const rec=(db.agentDays||[]).find(x=>String(x.id)===String(id));if(rec)openAgentDay(rec.agentId,rec.date);}
...
function renderAutoReportWizard(){
 ...
 if(autoReportWizardStep===0){wizardBody().innerHTML=`<div class="wizard-step"><h4>1. Comprendre le fonctionnement</h4><div class="wizard-status ok"><strong>Adresse qui reçoit les rapports :</strong><br>${esc(db.settings.autoReportTo||'adelin.vignal.running@outlook.fr')}</div><p>Cette adresse privée est le <strong>destinataire</strong>. Pour qu’un mail parte tout seul même lorsque l’application est fermée, Supabase doit utiliser un <strong>moteur d’envoi serveur</strong>.</p><div class="wizard-choice-grid"><button type="button" class="wizard-choice ${autoReportWizardData.provider==='microsoft'?'selected':''}" data-wizard-provider="microsoft"><strong>Microsoft 365 / Outlook professionnel</strong><small>Le serveur envoie depuis un compte professionnel autorisé par Microsoft 365.</small></button><button type="button" class="wizard-choice ${autoReportWizardData.provider==='resend'?'selected':''}" data-wizard-provider="resend"><strong>Resend</strong><small>Le serveur envoie depuis une adresse/domaine validé chez Resend.</small></button></div><div class="wizard-status warn">L’adresse privée Outlook.com ci-dessus ne doit pas être utilisée comme expéditeur Microsoft 365 automatique : elle sert ici à recevoir les rapports.</div></div>`}
 ...
 if(autoReportWizardStep===2){
  ...
  wizardBody().innerHTML=`<div class="wizard-step"><h4>3. Installer la fonction Supabase</h4><p>Depuis un ordinateur où Supabase CLI est installé, ouvrez un terminal dans le dossier de cette version et exécutez les commandes suivantes.</p><pre class="wizard-code" id="wizardCommands">${esc(cmds)}</pre><button type="button" class="ghost" id="copyWizardCommands">Copier les commandes</button><div class="wizard-status warn">Après le déploiement, exécutez aussi <strong>SETUP_CRON_EXEMPLE.sql</strong> dans SQL Editor en remplaçant le secret du cron par celui affiché ci-dessus.</div></div>`;
