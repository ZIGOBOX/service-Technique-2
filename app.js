...
async function downloadAttachment(id){
 ...
 // Secours V140 : certains anciens contrôles ont la pièce jointe dans la fiche métier
 // mais pas dans l'index global db.attachments. On la retrouve dans tous les modules.
 if(!rec){
  const pools=['periodic','issues','maintenance','requests','works','meetings','notes','documents','cleaning','roomPreps'];
  for(const key of pools){
   for(const row of (db[key]||[])){
    const found=(row.attachments||[]).find(a=>String(a.id)===String(id));
    if(found){rec={...found};db.attachments=db.attachments||[];if(!db.attachments.some(a=>String(a.id)===String(rec.id)))db.attachments.push(rec);break}
   }
   if(rec)break
  }
 }
...
function openAgent(id){
 ...
 openModal(old?'Modifier l’agent':'Nouvel agent',`<div class="form-grid">${field('Prénom','firstName',x.firstName,'text','required')}${field('Nom','lastName',x.lastName)}<label>Fonction<select name="role">${selectOptions(db.lists.roles,x.role)}</select></label>${field('Temps hebdomadaire (h)','weeklyHours',x.weeklyHours,'number','min="0" step="0.25"')}${field('Téléphone','phone',x.phone,'tel')}${field('E-mail','email',x.email,'email')}${field('Affectation principale','assignment',x.assignment)}<label>Statut<select name="status">${selectOptions(['Actif','Inactif'],x.status)}</select></label>${field('Date d’arrivée','arrivalDate',x.arrivalDate,'date')}<fieldset class="span2"><legend>Jours travaillés habituels</legend><p class="hint">Par défaut : lundi à vendredi. Décoche un jour pour qu'il apparaisse automatiquement en Repos.</p>${dayLabels.map(([label,d])=>`<label class="inline-check"><input type="checkbox" name="agentWorkday" value="${d}" ${x.workdays.includes(d)?'checked':''}>${label}</label>`).join('')}</fieldset>${textareaField('Notes','notes',x.notes)}${attachmentField(x.attachments)}</div>`,async form=>{
   ...
   await processAttachments(form,x,'agents');
...
function openAgentDay(agentId,date,id,preferredDayType=''){
 ...
 function refreshTheoretical(force=false){
   ...
   if(box){
     ...
     else {const nextPlan=(db.weeklyPlans||[]).filter(p=>String(p.agentId)===String(aid)&&p.effectiveFrom&&p.effectiveFrom>d).sort((a,b)=>a.effectiveFrom.localeCompare(b.effectiveFrom))[0];box.innerHTML=`<strong>Horaire théorique du ${fmtDate(d)}</strong><b>${sc.start&&sc.end?`${esc(sc.start)} – ${esc(sc.end)}`:'Aucun horaire applicable à cette date'}</b>${sc.pause?`<small>Pause : ${sc.pause} min</small>`:''}${sc.missions?`<small>${esc(sc.missions)}</small>`:''}${!sc.start&&nextPlan?`<small>Prochain planning enregistré à partir du ${fmtDate(nextPlan.effectiveFrom)}.</small>`:''}`;}
   }
...
function renderPeriodic(){
 const fam=$('#periodicFamily').value,status=$('#periodicStatus').value,bld=$('#periodicBuilding').value;
 const arr=db.periodic.filter(x=>(!fam||x.family===fam)&&(!status||periodicComputed(x)===status||x.status===status)&&(!bld||x.building===bld||x.building==='Tous bâtiments')).sort((a,b)=>(periodicDue(a)||'9999').localeCompare(periodicDue(b)||'9999'));
 $('#periodicCards').innerHTML=cardList(arr.map(x=>{
  const state=periodicComputed(x),ncs=(db.reportNonconformities||[]).filter(n=>String(n.periodicControlId||'')===String(x.id));
  const open=ncs.filter(n=>!['FAIT','Levée'].includes(n.status)),done=ncs.filter(n=>['FAIT','Levée'].includes(n.status)),plans=(db.issues||[]).filter(i=>ncs.some(n=>String(n.id)===String(i.sourceNonconformityId||''))),openPlans=plans.filter(i=>!isClosedStatus(i.status));
  const nc=ncs.length?`<section class="periodic-nc-block ${open.length?'has-open':'all-done'}"><div class="periodic-nc-summary"><strong>${open.length?'🔴 CONTRÔLE NON CONFORME':'🟢 OBSERVATIONS LEVÉES'}</strong><span>${ncs.length} observations · 🔴 ${open.length} à traiter · 🟢 ${done.length} FAIT/levées · 📋 ${openPlans.length} plan(s) d’action ouvert(s)</span></div>${open.length?`<h4>Non-conformités à traiter</h4>${open.map(n=>`<article class="periodic-nc-item"><div><strong>Observation ${esc(n.observationNo||'—')}</strong>${n.location?`<small>${esc(n.location)}</small>`:''}<p>${esc(n.text||'')}</p>${n.action?`<p><b>Préconisation :</b> ${esc(n.action)}</p>`:''}</div><select data-nc-status="${esc(n.id)}"><option selected>À traiter</option><option>FAIT</option><option>Levée</option></select></article>`).join('')}`:''}${done.length?`<details><summary>🟢 ${done.length} FAIT / levées</summary>${done.map(n=>`<article class="periodic-nc-item"><div><strong>Observation ${esc(n.observationNo||'—')} — ${esc(n.status)}</strong>${n.location?`<small>${esc(n.location)}</small>`:''}<p>${esc(n.text||'')}</p></div><select data-nc-status="${esc(n.id)}"><option>À traiter</option><option ${n.status==='FAIT'?'selected':''}>FAIT</option><option ${n.status==='Levée'?'selected':''}>Levée</option></select></article>`).join('')}</details>`:''}</section>`:'';
  const atts=(x.attachments||[]).filter(Boolean);
  const reportActions=atts.length?`<div class="periodic-report-actions">${atts.map(a=>`<button type="button" class="primary small" data-download="${esc(a.id)}">📄 Ouvrir ${esc(a.name||'le rapport PDF')}</button>`).join('')}</div>`:`<p class="hint">Aucun rapport PDF joint à ce contrôle.</p>`;
  return `<article class="periodic-card ${state==='En retard'?'late':''}" data-edit-type="periodic" data-edit-id="${esc(x.id)}"><div class="panel-head"><span>${esc(x.no)}</span>${badge(state)}</div><h3>${esc(x.name)}</h3><p>${esc(x.family)} · ${esc(x.building)}</p><dl><dt>Dernier</dt><dd>${fmtDate(x.lastDate)||'Non renseigné'}</dd><dt>Échéance</dt><dd>${fmtDate(periodicDue(x))||'À définir'}</dd><dt>Responsable</dt><dd>${esc(x.provider||'À définir')}</dd></dl>${nc}${reportActions}<div class="periodic-card-actions"><button type="button" class="ghost" data-edit-type="periodic" data-edit-id="${esc(x.id)}">✎ Ouvrir / modifier le contrôle</button></div></article>`;
...
function bindEvents(){
 $('#modalForm').addEventListener('submit',async e=>{
  ...
  btn.disabled=true;
  try{await modalHandler(form);if(pastDates.length&&changed.length){db.changeHistory=db.changeHistory||[];db.changeHistory.push({id:uid(),date:new Date().toISOString(),title:modalAuditTitle||'Modification d’une donnée passée',pastDates:[...new Set(pastDates)],changes:changed,user:currentUser?.email||'Utilisateur'});save()}}catch(err){
...
function refreshCentralImportValidation(){if(!centralImportAnalysis)return;$('#centralImportMeta').innerHTML=centralImportValidationHtml(centralImportAnalysis)}
...
async function saveScannedNote(){
 ...
 if(scannedNoteAttachment?.file){
   ...
   try{attachment=await putFile(f,{module:`scan-${destination}`,recordId:id});db.attachments.push(attachment)}
   catch(e){console.warn('Stockage Supabase du scan indisponible',e);try{const max=4*1024*1024;if(f.size<=max){const data=await new Promise((ok,ko)=>{const r=new FileReader();r.onload=()=>ok(r.result);r.onerror=ko;r.readAsDataURL(f)});attachment={id:uid(),name:f.name||'scan',type:f.type,size:f.size,data,module:`scan-${destination}`,recordId:id};db.attachments.push(attachment)}}catch(_){} }
 }
