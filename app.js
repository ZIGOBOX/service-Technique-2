...
const APP_BUILD='07/08/2026 06:45';
...
function migrate(raw){
 ...
 for(const k of ['buildings','spaces','agents','weeklyPlans','rotations','rotationExceptions','agentDays','personalEvents','issues','periodic','cleaning','maintenance','requests','works','meetings','notes','vacations','documents','contacts','attachments','archives','pdfImports','chronotimeDaily','chronotimeAnnual','reportNonconformities']){
...
function scheduledFor(agentId,date){
 ...
 return {shift,start:p?.start||(shift==='Matin'?r.morningStart:r.eveningStart),end:p?.end||(shift==='Matin'?r.morningEnd:r.eveningEnd),pause:Number(p?.pause??r.pause??0),missions:p?.missions||'',segments:p?.segments||[],source:'rotation'};
}
...
function renderDashboard(){const activeAgents=db.agents.filter(a=>a.status==='Actif'),present=activeAgents.filter(a=>!isAbsenceType(dayInfo(a.id,todayISO()).dayType)&&dayInfo(a.id,todayISO()).dayType!=='Repos').length;const openIssues=db.issues.filter(x=>!['Terminé','Clôturé'].includes(x.status)),lateIssues=openIssues.filter(x=>x.dueDate&&x.dueDate<todayISO()),urgent=openIssues.filter(x=>x.priority==='Urgente'||x.dueDate&&x.dueDate<=addDays(todayISO(),3));const openMaint=db.maintenance.filter(x=>!['Terminée','Clôturée'].includes(x.status)),todoMaint=openMaint.filter(x=>['À qualifier','À faire','Planifiée'].includes(x.status));const recentClean=db.cleaning.filter(x=>x.date>=addDays(todayISO(),-30)),comp=recentClean.length?Math.round(recentClean.filter(x=>x.overallStatus==='Conforme').length/recentClean.length*100):null,weak=recentClean.reduce((s,x)=>s+(x.tasks||[]).filter(t=>['À reprendre','Non conforme'].includes(t.status)).length,0);const pLate=db.periodic.filter(x=>periodicComputed(x)==='En retard'),pSoon=db.periodic.filter(x=>periodicComputed(x)==='Bientôt');const notes=db.notes.filter(x=>!['Terminé','Clôturé'].includes(x.status)),notesDue=notes.filter(x=>x.dueDate&&x.dueDate<=addDays(todayISO(),7)).length;$('#kpiAgents').textContent=activeAgents.length;$('#kpiPresent').textContent=`${present} présents aujourd’hui`;$('#kpiUrgentActions').textContent=urgent.length;$('#kpiLate').textContent=`${lateIssues.length} en retard`;$('#kpiMaintenance').textContent=openMaint.length;$('#kpiMaintenanceTodo').textContent=`${todoMaint.length} à faire`;$('#kpiCompliance').textContent=comp==null?'—':`${comp} %`;$('#kpiCleaningWeak').textContent=`${weak} point${weak>1?'s':''} faible${weak>1?'s':''}`;$('#kpiPeriodicLate').textContent=pLate.length;$('#kpiPeriodicSoon').textContent=`${pSoon.length} bientôt`;$('#kpiNotes').textContent=notes.length;$('#kpiNotesDue').textContent=`${notesDue} échéance${notesDue>1?'s':''} proche${notesDue>1?'s':''}`;
 const pri=[...urgent].sort((a,b)=>(a.dueDate||'9999').localeCompare(b.dueDate||'9999')).slice(0,5);$('#priorityList').innerHTML=cardList(pri.map(x=>itemCard('🔥',x.title,`${badge(x.priority)} · ${fmtDate(x.dueDate)||'Sans échéance'}`,'issue',x.id)),'Aucune urgence.');$('#dashboardNotes').innerHTML=cardList(notes.slice().sort((a,b)=>(a.dueDate||'9999').localeCompare(b.dueDate||'9999')).slice(0,5).map(x=>itemCard('✎',x.title,`${esc(x.category)} · ${fmtDate(x.dueDate)||'Sans échéance'}`,'note',x.id)),'Aucune note active.');$('#maintenancePreview').innerHTML=cardList(openMaint.filter(x=>['En cours','En attente prestataire','En attente pièce'].includes(x.status)).slice(0,5).map(x=>itemCard('⚙',x.title,`${esc(x.building)} · ${badge(x.status)}`,'maintenance',x.id)),'Aucune intervention en cours.');$('#maintenanceTodoPreview').innerHTML=cardList(todoMaint.slice(0,5).map(x=>itemCard('🧰',x.title,`${badge(x.priority)} · ${fmtDate(x.dueDate)||'Sans échéance'}`,'maintenance',x.id)),'Aucune intervention à faire.');const weakRows=[];recentClean.forEach(c=>(c.tasks||[]).filter(t=>['À reprendre','Non conforme'].includes(t.status)).forEach(t=>weakRows.push({c,t})));$('#cleaningWeakPreview').innerHTML=cardList(weakRows.slice(0,5).map(({c,t})=>itemCard('🧹',t.name,`${esc(c.building)} · ${esc(c.room)} · ${badge(t.status)}`,'cleaning',c.id)),'Aucun point faible récent.');const nextMeet=db.meetings.filter(x=>x.date>=todayISO()&&x.status!=='Annulé').sort((a,b)=>(a.date+a.time).localeCompare(b.date+b.time)).slice(0,5);$('#meetingPreview').innerHTML=cardList(nextMeet.map(x=>itemCard('📅',x.title,`${fmtDate(x.date)} ${esc(x.time||'')} · ${esc(x.location||'')}`,'meeting',x.id)),'Aucun rendez-vous à venir.');renderTeamCalendar();renderPersonalCalendar();window.PDFImportModule?.renderDashboard?.()}
...
function planningDisplayFor(agent,date){
 const info=dayInfo(agent.id,date),abs=isAbsenceType(info.dayType)||info.dayType==='Repos';
 const shift=String(info.shift||'').replace('Planning de référence','Standard');
 if(abs)return {text:info.dayType||'Repos',shift:'Repos',kind:'off'};
 if(info.dayType==='Formation')return {text:'Formation',shift:'Formation',kind:'info'};
...
function planningPrintCSS(){
 return `${reportPrintCSS('landscape')}.service-title{text-align:center;margin:0 0 12px}.service-title h1{font-size:18px;margin:0;color:#1f2937}.service-title p{margin:3px 0;color:#475569}.service-grid{border-collapse:collapse;width:100%;table-layout:fixed;font-size:9.5px}.service-grid th,.service-grid td{border:1px solid #6b7280;padding:5px 4px;text-align:center;vertical-align:middle}.service-grid th{background:#e5e7eb;color:#111827}.service-grid .agent-col{text-align:left;font-weight:700;width:18%}.service-grid td.shift-morning{background:#dbeafe}.service-grid td.shift-evening{background:#f3e8ff}.service-grid td.shift-standard{background:#dcfce7}.service-grid td.shift-off{background:#f3f4f6;color:#6b7280}.legend{display:flex;gap:12px;justify-content:center;margin:8px 0 12px;font-size:9px}.legend span{padding:3px 8px;border-radius:999px;border:1px solid #cbd5e1}.legend .m{background:#dbeafe}.legend .s{background:#f3e8ff}.legend .std{background:#dcfce7}.legend .off{background:#f3f4f6}.individual-grid{border-collapse:collapse;width:100%;font-size:10.5px}.individual-grid th,.individual-grid td{border:1px solid #9ca3af;padding:6px}.individual-grid th{background:#e5e7eb}.agent-summary{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin:10px 0 14px}.agent-summary div{border:1px solid #cbd5e1;border-radius:6px;padding:7px}.agent-summary small{display:block;color:#64748b}.agent-summary strong{display:block;margin-top:2px}`;
}
...
function generateIndividualPlanningPDF(){
 ...
 const rows=dates.map(d=>{const info=dayInfo(agent.id,d),x=planningDisplayFor(agent,d);return `<tr><td>${fmtDate(d)}</td><td>${parseDate(d).toLocaleDateString('fr-FR',{weekday:'long'})}</td><td>${esc(x.shift)}</td><td>${esc(x.text)}</td><td>${esc(info.dayType||'Présence')}</td><td>${esc(info.note||'')}</td></tr>`}).join('');
