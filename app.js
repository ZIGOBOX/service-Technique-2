...
function renderImportArchives(){
 ...
 box.innerHTML=rows.length?rows.map(x=>`<article class="import-archive-card"><div class="import-archive-icon">${x.type==='Chronotime'?'⏱':x.type==='Note scannée'?'📝':x.type.includes('Contrôle')||x.type.includes('Rapport')?'🛡':'📄'}</div><div class="import-archive-main"><div class="panel-head"><div><strong>${esc(x.fileName||x.subject||'Document importé')}</strong><small>${esc(x.type||'Document')} · ${x.createdAt?new Date(x.createdAt).toLocaleString('fr-FR'):'—'}</small></div>${x.academicYear?badge(x.academicYear):''}</div><p>${esc(x.subject||'')}</p><small>${esc(x.summary||'')}</small><div class="import-archive-actions">${x.attachmentId?`<button class="primary small" data-download="${esc(x.attachmentId)}">📄 Relire l’original</button>`:'<span class="muted">Original non stocké</span>'}${x.recordId?`<button class="ghost small" data-open-import-record="${esc(x.recordId)}" data-import-module="${esc(x.module||'')}">✎ Relire la fiche</button>`:''}${x.module?`<button class="ghost small" data-go="${esc(x.module)}">Ouvrir le module</button>`:''}</div></div></article>`).join(''):'<div class="empty-state">Aucun import ne correspond à ces filtres.</div>';
}
...
let centralImportAnalysis=null;
function resetCentralImport(){centralImportAnalysis=null;const f=$('#centralPdfFile');if(f)f.value='';$('#centralImportStart')?.classList.remove('hidden');$('#centralImportAnalysis')?.classList.add('hidden');$('#centralImportConfirm')?.classList.add('hidden');if($('#centralImportMeta'))$('#centralImportMeta').innerHTML='';}
...
function bindCentralImportV80(){
 ...
 $('#centralPdfFile')?.addEventListener('change',async e=>{const file=e.target.files?.[0];if(!file)return;$('#centralImportStart').classList.add('hidden');$('#centralImportAnalysis').classList.remove('hidden');$('#centralImportProgress').textContent='Analyse du PDF en cours…';$('#centralImportConfirm').classList.add('hidden');try{if(!window.PDFImportModule?.centralAnalyze)throw new Error('Moteur PDF indisponible');centralImportAnalysis=await window.PDFImportModule.centralAnalyze(file);const a=centralImportAnalysis;$('#centralImportType').value=a.detectedType;$('#centralImportMeta').innerHTML=`<div class="import-summary"><strong>${esc(file.name)}</strong><span>Type proposé : <b>${esc(a.detectedLabel)}</b></span><span>Confiance Chronotime : <b>${a.chronoConfidence}%</b></span><span>Confiance contrôle : <b>${a.controlConfidence}%</b></span><span>${esc(a.details||'Vérifiez le type avant de continuer.')}</span></div>`;$('#centralImportProgress').textContent='Analyse terminée — vérifiez puis confirmez.';$('#centralImportConfirm').classList.remove('hidden')}catch(err){console.error(err);$('#centralImportProgress').textContent='Impossible d’analyser ce PDF. Vous pouvez le classer manuellement comme document.';centralImportAnalysis={file,detectedType:'other'};$('#centralImportType').value='other';$('#centralImportConfirm').classList.remove('hidden')}});
 $('#centralImportConfirm')?.addEventListener('click',async()=>{if(!centralImportAnalysis)return;const type=$('#centralImportType').value,file=centralImportAnalysis.file;$('#centralImportModal').close();if(['chronotime','periodic','control'].includes(type)){window.PDFImportModule?.routeCentral?.(centralImportAnalysis,type);return}if(await genericImportedDocument(file,type)){setView('archives');toast('Document importé et archivé')}});
}
...
async function saveScannedNote(){
 ...
 db.importArchives.push({id:uid(),sourceId:record.id,createdAt:importedAt,type:archiveType,fileName:attachment?.name||scannedNoteAttachment?.name||record.title||record.name||'Scan',attachmentId:attachment?.id||'',subject:record.title||record.name||'',summary:text.slice(0,220),module:view,recordId:record.id});
