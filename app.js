...
const APP_BUILD='06/08/2026 00:01';
...
async function initAuth(){
 ...
 if(!window.supabase||!cfg.url||!cfg.publishableKey){$('#authError').textContent='Configuration Supabase absente.';return}
 supabaseClient=window.supabase.createClient(cfg.url,cfg.publishableKey);
 const {data:{session}}=await supabaseClient.auth.getSession();
 if(session?.user){await enterApp(session.user)}
 $('#authLogin')?.addEventListener('click',async()=>{
   const email=$('#authEmail').value.trim(),password=$('#authPassword').value;
   $('#authError').textContent='Connexion…';
   const {data,error}=await supabaseClient.auth.signInWithPassword({email,password});
   if(error){$('#authError').textContent=error.message==='Invalid login credentials'?'Adresse ou mot de passe incorrect.':error.message;return}
...
/* ---------- Navigation ---------- */
const VIEW_TITLES={dashboard:'Tableau de bord',personal:'Agenda personnel',agents:'Agents & recrutements',rotations:'Roulements annuels',planning:'Pilotage des horaires',absences:'Congés, RTT & absences',vacations:'Vacances & fermetures',issues:'Sécurité & qualité',periodic:'Contrôles périodiques',cleaning:'Contrôle ménage',maintenance:'Maintenance',requests:'Demandes direction',works:'Chantiers & GPA',meetings:'Réunions & rendez-vous',notes:'Bloc-notes',documents:'Documents & pièces jointes',archives:'Archives hebdomadaires',reports:'Rapports & impressions',settings:'Paramètres'};
...
function openAgentDay(agentId,date,id){
 ...
 const x=old||{id:uid(),agentId:initialAgentId,date:initialDate,dayType:'Présence',plannedStart:sched.start,plannedEnd:sched.end,actualStart:'',actualEnd:'',pause:sched.pause,overtime:0,status:'Validée',note:'',replacement:''};
  ...
  for(const day of days){
   ...
   for(const a of active){
    ...
    if(isAbsenceType(info.dayType)){
     abs.push({a,info,rec});
     if(!String(rec?.replacement||'').trim())push({level:'orange',icon:'⚠️',title:`${agentName(a)} absent${day===today?' aujourd’hui':` le ${fmtDate(day)}`}`,text:`${info.dayType||'Absence'} — remplacement à organiser`,view:'absences',type:rec?'agentDay':null,id:rec?.id||'',date:day});
...
function tableHTML(headers,rows){return `<table><thead><tr>${headers.map(h=>`<th>${esc(h)}</th>`).join('')}</tr></thead><tbody>${rows.length?rows.map(r=>`<tr>${r.map(c=>`<td>${c??''}</td>`).join('')}</tr>`).join(''):`<tr><td colspan="${headers.length}">Aucune donnée.</td></tr>`}</tbody></table>`}
...
function bindEvents(){
 ...
 $('#nav').addEventListener('click',e=>{const b=e.target.closest('[data-view]');if(b){setView(b.dataset.view);closeMobileMenu()}});$('#layoutMode').onchange=e=>applyLayout(e.target.value);$('#printCurrent').onclick=()=>window.print();
...
$('#archiveNow').onclick=()=>{const made=createWeeklyArchive(false);save();toast(made?'Archive créée':'La semaine précédente est déjà archivée')};$('#exportArchives').onclick=exportArchives;$('#exportBackup').onclick=exportBackup;$('#importBackup').onchange=e=>e.target.files[0]&&importBackup(e.target.files[0]);$('#resetData').onclick=resetData;const rr=$('#restoreReferenceData');if(rr)rr.onclick=restoreReferenceData;const dg=$('#runDiagnostic');if(dg)dg.onclick=runDiagnostic;$('#resetPeriodicCatalog').onclick=()=>{if(confirm('Restaurer le catalogue par défaut ? Les contrôles personnalisés actuels seront remplacés.')){db.periodic=makePeriodic();save()}};$('#exportCsv').onclick=()=>exportStyledExcel($('#csvModule').value);$('#exportRotationCsv').onclick=()=>exportStyledExcel('rotations');const ewp=$('#exportWeeklyPlans');if(ewp)ewp.onclick=()=>exportStyledExcel('weeklyPlans');
 ...
 $$('[data-report-print]').forEach(b=>b.onclick=()=>printReport(b.dataset.reportPrint));$$('[data-report-email]').forEach(b=>b.onclick=()=>prepareEmail(b.dataset.reportEmail));$('#printFullRegister').onclick=()=>printReport('full');$$('[data-print]').forEach(b=>b.onclick=()=>window.print());
