...
function centralCleaningRoomOptions(building,floor,type=''){
 ...
 if(!b)return [];
 const floorKey=v=>{const n=normalizeText(v);if(n==='rdc'||n.includes('rez de chaussee'))return 'rdc';const m=n.match(/(^|\s)([1-9])(?:er|e)?(?:\s|$)/);return m?`${m[2]}e`:n};
 const wantedFloor=floorKey(floor);
 const f=(b.floors||[]).find(x=>floorKey(x.name)===wantedFloor)||(b.floors||[]).find(x=>{
...
document.addEventListener('click',e=>{
 const p=e.target.closest?.('[data-periodic-open]');
 if(!p)return;
 e.preventDefault();e.stopImmediatePropagation();openPeriodic(p.dataset.periodicOpen);
},true);
