...
function migrate(raw){
 ...
 if(!raw||typeof raw!=='object'){mergeContractControls14723(base);migratePeriodicExcel2026V147161(base);migratePeriodicExcelHistoryV147162(base);migratePeriodicExcelFullV147163(base);ensureCanonicalFacilitySpaces(base);mergeBundledControlReports(base);return base;}
...
function renderDashboard(){updateLiveConnectionLocalStates();renderLiveConnections();renderGlobalAcademicYear();
 ...
 const pLate=(db.periodic||[]).filter(x=>normalizeText(periodicComputed(x))==='en retard'),pSoon=(db.periodic||[]).filter(x=>normalizeText(periodicComputed(x))==='bientot'),notes=(db.notes||[]).filter(x=>recordInAcademicYear(x,['date','dueDate'])&&!isClosedStatus(x.status)),notesDue=notes.filter(x=>{const due=recordDueDate(x);return due&&due<=addDays(todayISO(),7)}).length;
