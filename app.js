...
function calendarDayVisual(info){
 const raw=info.dayType||'Présence',day=normalizeText(raw),shift=normalizeText(info.shift||'Standard'),configured=db.settings?.chronoDayDisplay?.[raw];let base;
 if(/maladie/.test(day))base={cls:'sick',code:'MAL',label:'Maladie'};else if(/rtt/.test(day))base={cls:'rtt',code:'RTT',label:'RTT'};else if(/congé|conge/.test(day))base={cls:'leave',code:'C',label:'Congé'};else if(/férié|ferie|rfe/.test(day))base={cls:'holiday',code:'JF',label:'Jour férié'};else if(/repos/.test(day))base={cls:'off',code:'—',label:'Repos'};else if(/absence temps partiel/.test(day))base={cls:'other',code:'RTP',label:'Absence temps partiel'};else if(day!=='presence')base={cls:'other',code:day==='formation'?'F':'ABS',label:raw||'Absence'};else if(shift==='matin')base={cls:'morning',code:'M',label:'Matin'};else if(shift==='soir')base={cls:'evening',code:'S',label:'Soir'};else base={cls:'standard',code:'ST',label:'Standard'};
 if(configured){base.code=raw==='Absence temps partiel'?'RTP':(configured.abbr||base.code);base.color=configured.color||'';}return base;
}
...
async function saveScannedNote(){
 ...
 const scanArchive={id:uid(),sourceId:record.id,createdAt:importedAt,type:archiveType,fileHash:scanDuplicateInfo?.fileHash||'',fileName:attachment?.name||scannedNoteAttachment?.name||record.title||record.name||'Scan',attachmentId:attachment?.id||'',subject:record.title||record.name||'',summary:text.slice(0,220),module:view,recordId:record.id,analysisSnapshot:{type:archiveType,fileName:attachment?.name||scannedNoteAttachment?.name||record.title||record.name||'Scan',subject:record.title||record.name||'',summary:text.slice(0,220),destination:view,category,priority,text,createdAt:importedAt}};db.importArchives.push(scanArchive);if(attachment)registerImportOriginal(scanArchive,attachment);
