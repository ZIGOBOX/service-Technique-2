...
const serviceNorm=v=>String(v||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();
function roomPrepChoiceTone(text,index=0){const v=serviceNorm(text);if(v==='oui'||v.includes('pret')||v.includes('termine'))return'green';if(v==='non')return'slate';if(v.includes('preparation'))return'orange';return['cyan','blue','violet','teal','amber','rose'][index%6]}
function roomPrepChoiceIcon(id,text){const v=serviceNorm(text);if(id==='rpLayout'){if(v.includes('reunion'))return'🗣️';if(v.includes('classe'))return'🏫';if(v==='u')return'↪️';if(v.includes('conference'))return'🎤';if(v.includes('cocktail'))return'🥂';return'⋯'}if(['rpCoffee','rpPartition','rpOtherPrep'].includes(id))return v==='oui'?'✓':'×';if(id==='rpStatus'){if(v.includes('pret'))return'✓';if(v.includes('termine'))return'🏁';if(v.includes('preparation'))return'▶';return'⏱️'}return''}
function enhanceRoomPrepChoice(id){const select=$(id);if(!select||select.dataset.serviceEnhanced==='1')return;const opts=[...select.options].filter(o=>!o.disabled);if(opts.length<2||opts.length>12)return;const grid=document.createElement('div');grid.className='service-choice-grid roomprep-choice-grid';const buttons=opts.map((o,i)=>{const b=document.createElement('button');b.type='button';b.className='service-choice';b.dataset.value=o.value;b.dataset.tone=roomPrepChoiceTone(o.textContent,i);const icon=roomPrepChoiceIcon(id,o.textContent);if(icon){const sp=document.createElement('span');sp.className='service-choice-icon';sp.textContent=icon;b.append(sp)}const tx=document.createElement('strong');tx.textContent=o.textContent;b.append(tx);b.addEventListener('click',()=>{select.value=o.value;select.dispatchEvent(new Event('change',{bubbles:true}));syncRoomPrepChoice(id)});grid.append(b);return b});select.dataset.serviceEnhanced='1';select.classList.add('service-native-select');select.insertAdjacentElement('afterend',grid);select.addEventListener('change',()=>syncRoomPrepChoice(id));syncRoomPrepChoice(id)}
function syncRoomPrepChoice(id){const select=$(id);if(!select)return;const label=select.closest('label');label?.querySelectorAll('.service-choice').forEach(b=>{const active=String(b.dataset.value)===String(select.value);b.classList.toggle('active',active);b.setAttribute('aria-pressed',active?'true':'false')})}
function syncServiceChoices(){['rpStatus','rpLayout','rpPartition','rpOtherPrep','rpCoffee'].forEach(syncRoomPrepChoice)}
function enhanceRoomPrepStepper(id){const input=$(id);if(!input||input.dataset.serviceStepper==='1')return;const wrap=document.createElement('div');wrap.className='service-stepper';input.parentNode.insertBefore(wrap,input);wrap.append(input);const minus=document.createElement('button'),plus=document.createElement('button');minus.type=plus.type='button';minus.className=plus.className='service-stepper-btn';minus.textContent='−';plus.textContent='＋';wrap.insertBefore(minus,input);wrap.append(plus);input.dataset.serviceStepper='1';const move=dir=>{const step=Number(input.step)||1,min=input.min!==''?Number(input.min):-Infinity,max=input.max!==''?Number(input.max):Infinity;let val=Number(input.value)||0;val=Math.max(min,Math.min(max,val+dir*step));input.value=String(val);input.dispatchEvent(new Event('input',{bubbles:true}));input.dispatchEvent(new Event('change',{bubbles:true}))};minus.addEventListener('click',()=>move(-1));plus.addEventListener('click',()=>move(1))}
function enhanceServiceUI(){['rpStatus','rpLayout','rpPartition','rpOtherPrep','rpCoffee'].forEach(enhanceRoomPrepChoice);['rpPeople','rpTables','rpChairs','rpCoffeePeople'].forEach(enhanceRoomPrepStepper);syncServiceChoices()}
...
function editPrep(id){
 ...
 toggleCoffeeFields();updateEditorMode();syncServiceChoices();const st=$('roomPrepSaveStatus');if(st)st.textContent='Modification en cours';
...
enhanceServiceUI();reset();render();$('rpBuilding')?.addEventListener('change',()=>fillF());$('rpFloor')?.addEventListener('change',()=>fillS());$('rpSector')?.addEventListener('change',()=>fillR());$('rpRoom')?.addEventListener('change',toggleOther);$('rpOtherPrep')?.addEventListener('change',()=>$('rpOtherPrepCommentWrap').classList.toggle('hidden',$('rpOtherPrep').value!=='Oui'));document.querySelectorAll('[data-quick-roomprep]').forEach(x=>x.addEventListener('click',openPrep));$('openRoomPrepFromAgenda')?.addEventListener('click',openPrep);$('roomPrepPronote')?.addEventListener('click',pronote);document.addEventListener('click',e=>{
